...
public class TimeAttackScreen implements Screen, InputProcessor {
    final Squirgle game;
    ...
    public TimeAttackScreen(final Squirgle game) {
        this.game = game;
    ...
    public void render(float delta) {
        ...
        if(!paused) {
            ...
            if (!gameOver) {
                ...
                game.draw.drawTargetSemicircleTimeAttack(game.shapeRendererFilled);
            }
        }
        ...
        if(!paused) {
            if (!gameOver) {
                game.draw.drawInputButtonsTimeAttack(game, game.shapeRendererFilled);
                game.draw.drawScoreTriangleTimeAttack(game.shapeRendererFilled);
        ...
        if(!paused) {
            game.draw.drawTouchDownPointsTimeAttack(touchDownShapeList, game.shapeRendererLine);
        } else {
            drawInputRectangles();
        }
    ...
    public void drawTargetArc() {
        game.draw.drawArcTimeAttack(targetArcStart, targetArcColor, game.shapeRendererFilled);
    ...
    public void drawEquation() {
        if(!paused) {
            if(!gameOver) {
                if(equationWidth > 0) {
                    game.draw.drawEquationTimeAttack(lastShapeTouched, lastPromptShape, lastTargetShape, equationWidth, game.shapeRendererFilled);
    ...
    public void drawResultsInputButtons() {
        if(!paused) {
            if (showResults) {
                game.draw.drawResultsInputButtonsTimeAttack(INPUT_PLAY_SPAWN, INPUT_HOME_SPAWN, INPUT_EXIT_SPAWN, priorShapeList, game.shapeRendererFilled);
            }
        }
    }
    ...
    public void shapesMatchedBehavior() {
        ...
        if (targetShapesMatched == 1) {
            currentTargetShape.setColor(Color.WHITE);
    ...
    public void setUpNonFinalStaticData() {
        ...
        BACKGROUND_COLOR_SHAPE_LIST_MAX_HEIGHT = (game.camera.viewportHeight + BACKGROUND_COLOR_LIST_ELEMENT_RADIUS) + ((game.camera.viewportWidth - (TARGET_RADIUS * 2)) / 7);
        BACKGROUND_COLOR_SHAPE_LIST_MIN_HEIGHT = game.camera.viewportHeight + BACKGROUND_COLOR_LIST_ELEMENT_RADIUS;
